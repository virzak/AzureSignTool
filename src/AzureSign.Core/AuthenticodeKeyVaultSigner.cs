...
namespace AzureSign.Core
{
    ...
    public class AuthenticodeKeyVaultSigner : IDisposable
    {
        ...
        public unsafe int SignFile(ReadOnlySpan<char> path, ReadOnlySpan<char> description, ReadOnlySpan<char> descriptionUrl, bool? pageHashing, ILogger? logger = null, bool appendSignature = false)
        {
            ...
            ReadOnlySpan<byte> timestampAlgorithmOid = default;
            string? timestampUrl;
            ...
            fixed (char* pDescriptionUrl = NullTerminate(descriptionUrl))
            {
                ...
                {
                    cbSize = (uint)sizeof(SIGNER_FILE_INFO),
                    hFile = default,
                    pwszFileName = pPath,
                };
                var subjectIndex = 0u;
                ...
                {
                    cbSize = (uint)sizeof(SIGNER_SUBJECT_INFO),
                ...
                {
                    cbSize = (uint)sizeof(SIGNER_ATTR_AUTHCODE),
                    fCommercial = false,
                    fIndividual = false,
                ...
                {
                    cbSize = (uint)sizeof(SIGNER_CERT_STORE_INFO),
                ...
                {
                    cbSize = (uint)sizeof(SIGNER_CERT),
                ...
                {
                    cbSize = (uint)sizeof(SIGNER_SIGNATURE_INFO),
                ...
                {
                    cbSize = (uint)sizeof(SIGNER_DIGEST_SIGN_INFO),
                    // SIGNER_DIGEST_SIGN from mssign32.h; the Win32 metadata does not define it.
                ...
                switch (sipKind)
                {
                    case SipKind.Appx:
                        ...
                        flags &= ~SIGNER_SIGN_FLAGS.SPC_INC_PE_PAGE_HASHES_FLAG;
                        flags |= SIGNER_SIGN_FLAGS.SPC_EXC_PE_PAGE_HASHES_FLAG;
                        FillAppxExtension(ref clientData, flags, timeStampFlags, &subjectInfo, &signerCert, &signatureInfo, &pContext, pTimestampUrl, pTimestampAlgorithm, &signCallbackInfo);
                        break;
                }
                ...
                (
                    ...
                    null,
                    timeStampFlags,
                    new PCSTR(pTimestampAlgorithm),
                    new PCWSTR(pTimestampUrl),
                    null,
                    sipData,
                    &pContext,
                    null,
                    new[] { signCallbackInfo },
                    null
                );
